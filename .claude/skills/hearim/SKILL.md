...
| 조건                       | Tier       | 추가 수집                              |
| -------------------------- | ---------- | -------------------------------------- |
| config 없음 (NO_CONFIG)    | **Tier 1** | ~/Documents 자동 탐색 + 현재 레포 커밋 |
...
수집한 데이터에서 **개발 용어, 개념, 패턴**을 식별한다:

1. **개발 용어/개념 추출**: 커밋과 변경 파일에서 당일 접한 기술 용어, 개념, 패턴을 식별
2. **판단 기준 추출**: "왜 이 방법을 택했는가", "이 방법이 맞지 않는 경우는?" (일반화된 형태로)
3. **MEMORY.md 분석** (모든 Tier): 설계 결정사항에서 일반화 가능한 학습 포인트 추출
4. **이전 헤아림 연결**: 이전 "추가 학습 질의"에 대한 답을 오늘 찾았는지 확인
5. **반복 패턴 식별**: 여러 소스에서 반복 등장하는 주제 = 핵심 학습 포인트
...
- **Context -> Concepts** 순서: 먼저 배경+해결을 쓰고, 거기서 개념을 추출
...
- **공식 문서 연결**: 개념당 최소 1개의 공식 문서/권위 있는 레퍼런스 링크 포함
- **딥다이브**: 문서 맨 마지막에 "오늘의 키워드" 각각에 대해 FE 면접 Q&A 형태로 CS 지식을 정리. "Q: [면접 질문]" → 2-4문장 답변 (정의 + 왜 필요한지 + 내부 동작 원리). 키워드 전부 다룬다.

**톤 규칙:**

- **대외비 제거**: Jira 번호, 커밋 해시, 줄 수, 파일 수, 내부 서비스명/프로젝트명 제거
- **경험 기반 서사**: "내가 부딪힌 문제 -> 시도 -> 발견" 흐름으로 작성
- **정적 설명 금지**: "GPT에게 물어봐도 같은 답이 나오는 문장"은 내 경험으로 바꾼다
- 코드 예시는 내부 컴포넌트명을 일반화하되, 패턴의 본질은 유지
...
> 위클리는 스터디 발표 기록이므로, 데일리와 달리 **Git 커밋/Jira 자동 수집을 하지 않는다**.
> 사용자가 제공하는 발표 내용을 기반으로 작성한다.
