...
config가 없으면 **`~/Documents` 디렉토리 하위**에서 git 레포를 자동 탐색한다:
...
이 안내는 **매 실행마다 반복하지 않는다** — config가 없고, 이전 헤아림 데일리가 없을 때(즉 최초 사용자)만 표시한다. 이전 데일리가 이미 존재하면 사용자가 안내를 이미 본 것으로 간주한다.
